...
class App extends Component {
  ...
  insertEntry = async (item) => {
    const insertData = {
      ...item,
      birthDate: new Date(item.birthDate).toISOString().replace('.000Z', '')
    };
        ...
        <pre>
          {datas.map((data, i) => (
            <li key={i} className="myList">
              {i + 1}. {data.name}, {data.address}, {data.birthDate},
              {data.number},{data.vaccinated ? 'Vaccinated' : 'Not vaccinated'}
              <button
                onClick={() => this.fRemove(data.id)}
                className="myListButton"
              >
                remove{' '}
              </button>
              <button
                onClick={() => this.fEdit(data.id)}
                className="myListButton"
              >
                edit{' '}
              </button>
